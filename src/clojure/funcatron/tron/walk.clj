...
Updated to support Java collections
"}
  funcatron.tron.walk
...
(defn walk
  ...
  [inner outer form]
  (cond
    (or (list? form) (instance? List form)) (outer (apply list (map inner form)))
    (instance? java.util.Map$Entry form) (outer (vec (map inner [(.getKey ^java.util.Map$Entry form)
                                                                 (.getValue ^java.util.Map$Entry form)])))
...
(defn keywordize-keys
  ...
  (let [f (fn [[k v]] (if (string? k) [(keyword k) v] [k v]))]
    ;; only apply to maps
    (postwalk (fn [x] (if (or (map? x) (instance? Map x)) (into {} (map f x)) x)) m)))
